...
var script = {
  ...
  methods: {
    ...
    registerColumn: function registerColumn(column) {
      if (this.registerColumns && column.type.name === 'Column') {
        this.d_registeredColumns.push(column);
      }
    },
    unregisterColumn: function unregisterColumn(column) {
      this.d_registeredColumns = this.d_registeredColumns.filter(function (c) {
        return c !== column;
      });
    }
  },
  computed: {
    columns: function columns() {
      ...
      if (this.registerColumns) {
        cols.push.apply(cols, _toConsumableArray(this.d_registeredColumns));
      } else if (children) {
        cols.push.apply(cols, _toConsumableArray(this.recursiveGetChildren(children, [])));
      }
      if (!cols.length) {
        return;
      }
